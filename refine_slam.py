...
class PtzSlam:
    def __init__(self, model_path, annotation_path, data_path):
        ...
        """
        self.u, self.v = self.annotation[0][1]['camera'][0][0:2]
        ...
        pan, tilt, f = self.annotation[0][1]['ptz'].squeeze()
        self.x[0] = [pan, tilt, f]
    ...
    def compute_jacobi_approximate(theta, phi, foc, ray):
        ...
        jacobi_h[1][1] = (-foc * math.tan(ray[1] - (phi + delta)) + foc * math.tan(ray[1] - (phi - delta))) / (
                2 * delta)
        jacobi_h[1][2] = (-(foc + delta) * math.tan(ray[1] - phi) + (foc - delta) * math.tan(ray[1] - phi)) / (
                2 * delta)
        return jacobi_h
    ...
    def get_observation_from_ptz(self, pan, tilt, f):
        ...
        for j in range(len(self.rays)):
            pos = np.array(self.pts[j])
            features[j] = synthesize.from_3d_to_2d(self.u, self.v, f, math.radians(pan), math.radians(tilt), self.c,
    ...
    def extended_kalman_filter(self, previous_x, previous_p, observe):
        ...
        for i in range(len(index)):
            ...
            jacobi[2 * i:2 * i + 2, 0:3] = PtzSlam.compute_jacobi(predict_x[0], predict_x[1], predict_x[2],
                                                                  self.rays[pts_index])

        s = np.dot(np.dot(jacobi, predict_p), jacobi.T) +0.01 * np.ones([2 * len(index), 2 * len(index)])
